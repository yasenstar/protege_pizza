...
In the previous sections, we introduced **Description Logic (DL)** as the formal knowledge representation language underlying OWL, and examined the first DL constructor introduced in Exercise 15: the **existential restriction (`some`)**.
...
Description Logic provide precisely this capability through **class expressions**.

Rather than describing concepts using isolated restrictions, class expressions allow ontology engineers to compose multiple logical conditions into increasingly expressive semantic descriptions while preserving mathematical precision and machine interpretability.

This represents another important milestone in the Semantic Knowledge Development Lifecycle (SKDL). During **Stage 1 -- Conceptual Modeling**, we organized concepts into taxonomies. During **Stage 2 -- Semantic Description**, we begin expressing what those concepts actually mean by constructing logical descriptions that machines can interpret consistently.
...
:Named Class\nMargheritaPizza;
->described by;
:NamedPizza\nand\n(hasTopping som MozzarellaTopping)\nand\n(hasTopping some TomatoTopping);
@enduml
```
...
Consequently, class expressions should not be viewed as isolated pieces of OWL syntax. They are modular semantic building blocks that allow an ontology to evolve steadily from conceptual organization into a rich snd formally defined knowledge model.
...
Every restriction, every logical constructor, and every class expression introduced throughout this chapter can be interpreted formally using the language of **set theory** and **first-order logic (FOC)**.
...
contained within this relation states that pizza **$p$** possesses topping **$t$**.
...
> for which there exists at least on topping **y**
> such that
> **x** has topping **y**
...
- a domain of individuals: $\Delta^I$, and
...
$Pizza^I \subseteq \Delta^I$,

while, `hasTopping` is mapped to:

$hasTopping^I \subseteq \Delta^I \times \Delta^I$
