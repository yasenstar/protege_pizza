...
- [17.7 Existential Restrictions (`some`) -- The First Description Logic Constructor](#177-existential-restrictions-some----the-first-description-logic-constructor)
  - [17.7.1 From Exercise 15 to Description Logic](#1771-from-exercise-15-to-description-logic)
...
One of the most important ideas introduced in Exercise 15 is that OWL is no longer merely organizing concepts into a hierarchy.

For the first time, we begin describing the semantic characteristics that distinguish one concept from another.

This semantic description is achieved through **existential restrictions**, represented in OWL by the keyword `some`.

Although the syntax appears simple, `some` is one of the most fundamental constructors in **Description Logic (DL)** and forms the basis of countless ontology definitions.

### 17.7.1 From Exercise 15 to Description Logic

In Exercise 15, we created the following restriction for `MargheritaPizza`:

```text
hasTopping some MozzarellaTopping
```

A second restriction was then added:

```text
hasTopping some TomatoTopping
```

Within Protégé these expressions are entered using the **Class Expression Editor**, but semantically they correspond directly to Description Logic.
