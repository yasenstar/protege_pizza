...
- [13.4 Range - Defining Valid Relationships Targets](#134-range---defining-valid-relationships-targets)
- [13.5 Configuring Domain and Range in Protégé](#135-configuring-domain-and-range-in-protégé)
  ...
  - [13.5.2 Multi-Domain and Multi-Range Configurations](#1352-multi-domain-and-multi-range-configurations)
    - [Modeling `OR` Semantics in Domain and Range](#modeling-or-semantics-in-domain-and-range)
      - [Options 1: Use `owl:unionOf` (Recommended)](#options-1-use-owlunionof-recommended)
      - [Options 2: Create Separate Properties](#options-2-create-separate-properties)
      - [Option 3: Use Local `SubClassOf` Restrictions](#option-3-use-local-subclassof-restrictions)
    - [Summary: Choosing the Right Approach](#summary-choosing-the-right-approach)
    - [Risks of Careless Multi-Domain and Multi-Range Modeling](#risks-of-careless-multi-domain-and-multi-range-modeling)
...
- [13.7 Practical Modeling in `Pizza.owl` - Applying Domain and Range to Inverse Properties](#137-practical-modeling-in-pizzaowl---applying-domain-and-range-to-inverse-properties)
  ...
  - [13.7.6 Applying the Same Semantic Pattern to `hasBase` -- Exercise 12](#1376-applying-the-same-semantic-pattern-to-hasbase----exercise-12)
  - [13.7.7 Look back of Two Exercises](#1377-look-back-of-two-exercises)
  - [13.7.8 Why This Matters Beyond `Pizza.owl`](#1378-why-this-matters-beyond-pizzaowl)
- [13.8 EKA Tuple Mapping -- Domain and Range in Executable Knowledge Architecture](#138-eka-tuple-mapping----domain-and-range-in-executable-knowledge-architecture)
  ...
  - [13.8.4 $\\Phi$ - Actions *(Future EKA Perspective)*](#1384-phi---actions-future-eka-perspective)
...
- [13.13 Exercise 12 Transition -- From Domain and Range to Semantic Restrictions](#1313-exercise-12-transition----from-domain-and-range-to-semantic-restrictions)
- [13.14 Key Concepts](#1314-key-concepts)
- [13.15 Protégé Skills Learned](#1315-protégé-skills-learned)
- [13.16 Next Chapter Preview](#1316-next-chapter-preview)
- [13.17 One more thing...](#1317-one-more-thing)
...
A common modeling question arises when a property legitimately needs to point to **one of several possible types** -- for example, a `realizedBy` property where the target can be either a `Business_Process` **or** and `Application` (but not necessarily both).
...
![unionOf](img/ch13_12_unionOf.png)
...
```xml
    <!-- http://www.semanticweb.org/v0cn037/ontologies/2026/7/untitled-ontology-25#realized_By_Union -->

    <owl:ObjectProperty rdf:about="http://www.semanticweb.org/v0cn037/ontologies/2026/7/untitled-ontology-25#realized_By_Union">
        <rdfs:domain rdf:resource="http://www.semanticweb.org/v0cn037/ontologies/2026/7/untitled-ontology-25#Business_Capability"/>
        <rdfs:range>
            <owl:Class>
                <owl:unionOf rdf:parseType="Collection">
                    <rdf:Description rdf:about="http://www.semanticweb.org/v0cn037/ontologies/2026/7/untitled-ontology-25#Application"/>
                    <rdf:Description rdf:about="http://www.semanticweb.org/v0cn037/ontologies/2026/7/untitled-ontology-25#Business_Process"/>
                </owl:unionOf>
            </owl:Class>
        </rdfs:range>
    </owl:ObjectProperty>
...
![ch13_13_realizedByBusinessProcess](img/ch13_13_realizedByBusinessProcess.png)

![ch13_14_realizedByApplication](img/ch13_14_realizedByApplication.png)
...
![ch13_15_local-realizedBy](img/ch13_15_local-realizedBy.png)

This pops up the scope comparison between **Global** and **Local**:

| Approach | Scope | Inference Behavior |
|----------|-------|---------------------|
| `rdfs:range` | **Global** — applies to every use of the property | Inferred for all subjects |
| `SubClassOf some` restriction | **Local** — applies only to instances of that class | Inferred only for those instances |
...
```xml
    <!-- http://www.semanticweb.org/v0cn037/ontologies/2026/7/untitled-ontology-26#Business_Capability -->

    <owl:Class rdf:about="http://www.semanticweb.org/v0cn037/ontologies/2026/7/untitled-ontology-26#Business_Capability">
        <rdfs:subClassOf>
            <owl:Restriction>
                <owl:onProperty rdf:resource="http://www.semanticweb.org/v0cn037/ontologies/2026/7/untitled-ontology-26#realizedBy"/>
                <owl:someValuesFrom rdf:resource="http://www.semanticweb.org/v0cn037/ontologies/2026/7/untitled-ontology-26#Application"/>
            </owl:Restriction>
        </rdfs:subClassOf>
        <rdfs:subClassOf>
            <owl:Restriction>
                <owl:onProperty rdf:resource="http://www.semanticweb.org/v0cn037/ontologies/2026/7/untitled-ontology-26#realizedBy"/>
                <owl:someValuesFrom rdf:resource="http://www.semanticweb.org/v0cn037/ontologies/2026/7/untitled-ontology-26#Business_Process"/>
            </owl:Restriction>
        </rdfs:subClassOf>
    </owl:Class>
```
...
![ch13_11_isBaseOf-reasoning](img/ch13_11_isBaseOf-reasoning.png)
...
### 13.8.4 $\Phi$ - Actions *(Future EKA Perspective)*
...
Through **$\Gamma$**, the semantic knowledge becomes governed through mechanisms for semantic discipline, validation, and, where required, enforcement.

Later, **$\Theta$** and **$\Phi$** can connect governed semantic conditions to triggers and actions.

The progression can therefore be viewed as:

**$K/R \rightarrow \Gamma \rightarrow \Theta/\Phi$**

where **$\Gamma$** serves as the important bridge between **semantic interpretation** and **controlled operational use**.
...
![EKA Roadmap](img/EKA-Roadmap.png)
...
| --- | --- |
|**`owl:unionOf` range** | Semantically clean, reusable, tells reasoner the target can be one of several types |
| **Separate properties** | Clearer semantics, easier governance, but more properties to maintain |
| **Local restrictions** (`SubClassOf some`) | Avoids global side effects; good for modular design |
...
- For a **Data Property**, the Range is typically a concrete datatype (e.g., `xsd:string`, `xsd:integer`, `xsd:date`) rather than an OWL class. For example, `hasCalories` (Data Property) can be modeled with a Domain of `Pizza` and a Range of `xsd:integer`, communicating that only pizzas have a calorie count represented as an integer value.
