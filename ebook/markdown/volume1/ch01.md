...
> [!Note]
> **Graph Technology Clarification:** In the EKA roadmap, the "Knowledge Graph" layer can be implemented as either a Native RDF Graph Database (e.g., GraphDB, AllegroGraph, Stardog) or a Property Graph (e.g., Neo4j). The choice has profound implications for preserving OWL semantics and reasoning capabilities. For a full EKA implementation that leverages executable semantics, a Native RDF Graph is recommended.
