...
- [4.8 Key Concepts](#48-key-concepts)
- [4.9 Protégé Skills Learned](#49-protégé-skills-learned)
- [4.10 Next Chapter (05) Preview](#410-next-chapter-05-preview)


## 4.1 Understanding Classes in Ontology

In OWL, **classes** represent **conceptual categories** -- abstract groupings of things that share common characteristics. They answer the typical question:
...
3. Create `Topping` Class:
   
   We will have `Topping` at the same level as `Pizza` class, so depending on your viewpoint, `Topping` can be treated as a subclass of root `owl:Thing` class, or a sibling class of just created `Pizza` class.

   If you select `owl:Thing`, then click `Add subclass` icon:

   ![ch04_08_topping-as-subclass](img/ch04_08_topping-as-subclass.png)

   If you select `Pizza`, then click `Add sibling class` icon (only now it's clickable):

   ![ch04_09_topping-as-siblingclass](img/ch06_09_ontology-in-red.png)

   The pop-up dialogs are same, just type `Topping` and click `OK`, your ontology should be looked as below:

   ![ch04_10_pizza-and-topping](img/ch04_10_pizza-and-topping.png)
