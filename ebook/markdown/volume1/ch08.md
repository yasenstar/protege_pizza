...
| --- | --- |
| Preserve OWL semantics for reasoning | **native RDF Graph** (AllegroGraph, GraphDB, Stardog) |
| Execute complex, relationship-heavy queries (e.g., pathfinding) | **Property Graph** (Neo4j, TigerGraph) |
| Integrate with Lined Data and the Semantic Web | **Native RDF Graph** |
...
> [!important]
> **EKA Recommendation:** For building a semantic layer over enterprise data, to leverage reasoning, and to utilize Linked Data, a **Native RDF Graph Databases** is the correct architectural choice for the EKA $K$ layer. A Property Graph like Neo4j can be used as a downstream, specialized cache or execution layer for specific, performance-intensive queries, but it should not be the primary $K$ layer for a system that depends on OWL semantics.
