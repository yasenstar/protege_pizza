...
- [2.7 Building Class Hierarchies Systematically](#27-building-class-hierarchies-systematically)
...
- [2.19 Next Chapter (03) Preview](#219-next-chapter-03-preview)
- [2.20 Reference](#220-reference)
- [2.21 Demo Video for this Chapter](#221-demo-video-for-this-chapter)
...
From Wikipedia, [**Taxonomy**](https://en.wikipedia.org/wiki/Taxonomy) is a practice and science concerned with classification or categorization.
...
> [!Note]
> **Graph Technology Clarification:** When these ontology structure are later exported to a graph database, the choice of graph technology matters.
> - A **Native RDF Graph Database** (e.g., allegroGraph, Stardog, GraphDB) can load the ontology directly and preserve all its semantic meaning.
> - A **Property Graph** (e.g., Neo4j) requires a lossy transformation that sacrifices OWL reasoning capabilities.
>
> For EKA Implementations, a Native RDF Graph is the recommended choice for the primary $K$ layer.
...
| **Class** | A category or type of thing in an ontology (e.g., `Pizza`, `PizzaTopping`). Classes can be organized into hierarchies to represent semantic inheritance. |
| **Subclass** | A class that inherits characteristics from a parent class. For example, `VegetarianPizza` is a subclass of `Pizza`. |
| **Disjoint Class** | Classes that are declared mutually exclusive — an individual cannot belong to two disjoint classes simultaneously (e.g., `MeatTopping` and `VegetableTopping`). |
...
| **Taxonomy** | A hierarchical classification structure that organizes concepts into parent-child relationships (e.g., `PizzaTopping` → `CheeseTopping`). A taxonomy provides the structural foundation for an ontology. |
| **Necessary Condition** | A condition that must be true for an individual to belong to a class. For example, all `Pizza` must have a base. |
| **Sufficient Condition** | A condition that, if satisfied, is enough to classify an individual into a class. For example, if a pizza contains only vegetable toppings, it can be inferred to be `VegetarianPizza`. |
