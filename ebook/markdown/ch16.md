...
The practical task is intentionally modest. Rather than constructing an extensive taxonomy containing numerous pizza varieties, the exercise introduces only tow new classes:

- **NamedPizza**
- **MargheritaPizza**

At first glance, this may appear surprisingly simple. However, this simplicity is intentional and reflect a sound ontology engineering methodology.
...
These richer semantic characteristics will be introduced progressively during hte following stages of the Semantic Knowledge Development Lifecycle.
...
The ontology is no longer expanding by adding isolated concepts, this incremental engineering strategy offers several important advantages.
...
Although Exercise 14 contains only a small amount of modeling work, it establishes the conceptual vocabulary upon which the remainder of the `Pizza` ontology will be constructed. Every named pizza introduced in subsequent exercises will ultimately become a specialization of the abstraction created in this chapter, demonstrating how a carefully designed conceptual hierarchy supports the incremental evolution of a semantic knowledge system.
