...
Conceptual Modeling is far more than creating classes within Protégé.
...
This distinction need to be carefully acknowledged especially when you change from one visualizing diagraming tool (e.g. Microsoft Visio, draw.io) to Protégé.
...
Every `MargheritaPizza` is a (**IS-A**) `NamedPizza`, and every `NamedPizza` is a `Pizza`.
...
Whenever several concepts share common characteristics, introduce a higher-level appropriate abstraction instead of repeatedly modeling similar structures.
