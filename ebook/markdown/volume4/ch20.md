...
Object-oriented inheritance is the closest software analog to `SubClassOf` ([Section 20.5.1](#2051-inheritance-subclassof)): a subclass inherits behavior from its parent without restating it, exactly as `VegetarianPizza` inherits every axiom that applies to `Pizza`.

Using PlantUML, you may illustrate Parent-Child inheritance relationship via below class diagram, with syntax `Parent <|-- Child`:
