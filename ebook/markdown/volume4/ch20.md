...
This is inheritance in the same sense object-oriented programmers use the word, but with an important semantic difference.

- In software engineering, inheritance reuses **behavior**.
- In OWL, inheritance reuses **meaning**.

A subclass does not merely borrow methods. It inherits the full logical commitment of its parent class -- everything the reasoner knows about `Pizza` becomes, *automatically and without restatement*, something the reasoner also knows about `VegetarianPizza`.
...
`SubClassOf` states a **necessary condition**, not a **necessary-and-sufficient** one.
