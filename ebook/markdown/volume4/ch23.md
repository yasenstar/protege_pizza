...
- [23.10 Reasoning in Industrial Contexts](#2310-reasoning-in-industrial-contexts)
  ...
  - [23.10.7 Deeper Industrial Semantic Context -- Grounding Reasoning in the ISA-95 Standard](#23107-deeper-industrial-semantic-context----grounding-reasoning-in-the-isa-95-standard)
    - [23.10.7.1 The ISA-95 Functional Hierarchy and Time Horizons](#231071-the-isa-95-functional-hierarchy-and-time-horizons)
    - [23.10.7.2 Why ISA-95 Object Models Matter for Ontology Reasoning](#231072-why-isa-95-object-models-matter-for-ontology-reasoning)
    - [23.10.7.3 Reasoning Over Industrial Hierarchies in Practice](#231073-reasoning-over-industrial-hierarchies-in-practice)
- [23.11 EKA Perspective -- Stage 6 Establishes the Reasoning Layer ($R$)](#2311-eka-perspective----stage-6-establishes-the-reasoning-layer-r)
  - [23.11.1 Reasoning and the EKA Tuple](#23111-reasoning-and-the-eka-tuple)
  - [23.11.2 The Contribution of Stage 6 to EKA](#23112-the-contribution-of-stage-6-to-eka)
  - [23.11.3 From $K$ to $R$: The Transition to Intelligence](#23113-from-k-to-r-the-transition-to-intelligence)
  - [23.11.4 Reasoning as the Bridge to Execution](#23114-reasoning-as-the-bridge-to-execution)
- [23.12 Engineering Guidelines for Semantic Reasoning](#2312-engineering-guidelines-for-semantic-reasoning)
- [23.13 Key Concepts](#2313-key-concepts)
- [23.14 Chapter Summary](#2314-chapter-summary)
- [23.15 Looking Ahead -- Tower Executable Knowledge](#2315-looking-ahead----tower-executable-knowledge)
...
### 23.10.7 Deeper Industrial Semantic Context -- Grounding Reasoning in the ISA-95 Standard

As ontology engineering expands from abstract domains into industrial manufacturing, energy, and supply chain automation, a critical challenge emerges:

> **how to connect real-time sensor data and control systems within higher-level semantic reasoning?**

An automated reasoner **cannot** evaluate operational efficiency, equipment safety, or batch quality if it operates in a vacuum. If requires a standardized framework that defines how physical assets, materials, personnel, and processes relate to one another.

In industrial automation, this framework is universally provided by **ISA-95 (ANSI/ISA-95 / IEC 62264)**, the international standard for enterprise-control system integration.

#### 23.10.7.1 The ISA-95 Functional Hierarchy and Time Horizons

The ISA-95 standard structures manufacturing enterprises into a functional hierarchy spanning from physical processes up to business planning:

- **Level 0 -- The Physical Process**: The actual raw materials, chemical reactions, and physical machinery.
- **Level 1 & Level 2 -- Sensing, Manipulation, and Control**: PLCs, actuators, sensors, and SCADA systems operating in real-time milliseconds.
- **Level 3 -- Manufacturing Operations Management (MOM / MES)**: Production, quality, maintenance, and inventory management operating on event- and shift-driven horizons.
- **Level 4 -- Business Planning and Logistics (ERP)**: Enterprise resource planning operating on weekly, monthly, or quarterly horizons.

While Level 1 and Level 2 handle high-frequency, real-time control, **Level 3 and Level 4 require semantic reasoning** to aggregate, contextualize, and translate raw telemetry into business-meaningful events.

#### 23.10.7.2 Why ISA-95 Object Models Matter for Ontology Reasoning

A major bottleneck in industrial AI and knowledge graph projects is the **lack of a common vocabulary**. Different systems use conflicting identifiers for the same physical assets.

ISA-95 solves this by defining standardized object models for:

- **Equipment Hierarchies**: Enterprise $\to$ Site $\to$ Area $\to$ Work Center $\to$ Work Unit $\to$ Equipment Module
- **Physical Assets**: Machinery, tools, and instruments
- **Materials**: Raw components, intermediate work-in-progress (WIP), and finished products
- **Personnel**: Roles, competencies, and labor tracking

When we build industrial ontologies within the **Executable Knowledge Architecture (EKA)** framework, mapping our classes and object properties directly to ISA-95 object models ensures that our semantic descriptions match standard industrial data structures (often serialized via B2MML or integrated through OPC UA companion specifications).

#### 23.10.7.3 Reasoning Over Industrial Hierarchies in Practice

Consider an industrial scenario where a machine sensor reports an anomaly.

A flat database sees only a tag value (e.g., `Sensor_402_Temp = 95°C`).

An ISA-95-aligned ontology combined with Description Logic (DL) reasoning transforms this data point into executable knowledge, mapping to stages in SKDL:

1. **Structural Inheritance (Stage 1 & 2)**: The reasoner knows via the ISA-95 equipment hierarchy that `Reactor_01` is a member of `WorkUnit`, which belongs to `Polymerization_Area`.
2. **Semantic Description (Stage 2)**: The ontology defines that any `WorkUnit` assigned to a high-temperature process must respect specific thermal limits.
3. **Automated Reasoning (Stage 6)**: When the temperature threshold breached, the reasoner automatically classified the asset not merely as a failing sensor, but as an *Affected Production Resource* impacting the current active `productionOrder`.

By grounding industrial reasoning in ISA-95 models, the ontology bridges the gap between the shop floor and enterprise logic. It turns thousands of disparate telemetry points into a coherent queryable, and logically consistent semantic model ready for automated decision-making and operational execution.

Production platforms such as Rhize (https://rhize.com/) implement this pattern directly, using ISA-95 as the native schema of a real-time, event-driven manufacturing data hub -- a concrete example of the reasoning-to-execution pipeline described throughout this chapter (see Info Box, below).

> **Info Box -- Industrial Practice: Rhize Manufacturing Data Hub**
>
> Rhize (https://rhize.com/) is a real-time, event-driven manufacturing data hub built directly on the ISA-95 standard. Rather than treating ISA-95 as a documentation convention to be mapped after the fact, Rhize implements it as the **native schema** of its data platform: equipment hierarchies, material, personnel, and production events are all contextualized in an ISA-95-aligned model from the moment data is ingested.
>
> Three architectural choices make Rhize a compelling production-grade illustration of the reasoning principles discussed in this chapter:
>
> - **ISA-95 as the ontology, not the mapping.** Rhize's headless platform ingests data from edge to ERP to quality systems and stitches it into a standardized ISA-95 schema. This is precisely the "grounding" move described in [Section 23.10.7.2](#231072-why-isa-95-object-models-matter-for-ontology-reasoning) -- the ontology is not a separate artifact that must be reconciled with operational data; it ***is*** the operational data structure.
> - **Event-driven contextualization.** Rhize uses rules and custom business logic to contextualize raw telemetry into real-time events. This is the reasoning layer in operation: raw sensor readings become classified events (equipment stoppage, operator action, quality deviation) that downstream systems can act upon.
> - **GraphQL over a standardized schema.** Applications query Rhize's ISA-95-aligned model through a single GraphQL API, with query subscriptions supporting real-time feeds. This is a production illustration of the $K \to R$ pipeline that ultimately feeds $\Theta/\Phi$: the knowledge graph ($K$) is queried through reasoning-informed classifications ($R$), which trigger events ($\Theta$) and drive workflows ($\Phi$).
>
> Rhize is not a Protégé reasoner, and it does not use OWL DL in the academic sense. But it demonstrates the same engineering principle this chapter has developed: **a standardized semantic model, combined with rule-based contextualization, transforms raw data into executable knowledge.** The ISA-95 hierarchy provides the structural inheritance; the event rules provide the reasoning; the GraphQL API provide the query and subscription surface that makes the result actionable.
>
> For readers building industrial knowledge graphs, Rhize is a useful reference architecture: it shows what it looks like when ISA-95 is treated as a living ontology rather than a compliance document, and when reasoning is embedded in the data platform rather than bolted on as a separate reasoning service.

## 23.11 EKA Perspective -- Stage 6 Establishes the Reasoning Layer ($R$)

Throughout this chapter, we have explored reasoning from **practical**, **mathematical**, **historical**, and **industrial** perspective.

Now we turn to the architectural perspective:

> *how does reasoning fit into the Executable Knowledge Architecture?*

### 23.11.1 Reasoning and the EKA Tuple
...
Earlier chapters mapped their contributions to this tuple explicitly as below:

- **Chapter (18)** (Stage 1 -- Conceptual Modeling) establishes the foundational structure of $K$.
- **Chapter (19)** (Stage 2 -- Semantic Description) enriches $K$ with formal meaning and prepared the logical ground for $R$.
- **Chapter (20)** (Stage 3 -- Knowledge Reuse) strengthened $K$ and prepared $\Gamma$.
- **Chapter (21)** (Stage 4 -- Semantic Governance) establishes $\Gamma$ -- the Governance layer.
- **Chapter (22)** (Stage 5 -- Semantic Validation) verifies $\Gamma + K$ -- the correctness of the governed knowledge graph.
- **Chapter (23)** (Stage 6 -- Semantic Reasoning) now establishes $R$ -- the Reasoning & Rules layer.

### 23.11.2 The Contribution of Stage 6 to EKA

The contribution of **Stage 6** to each EKA component can be summarized as follows:

| EKA Component | Contribution of Stage 6 (Semantic Reasoning) |
| --- | --- |
| **$K$ -- Knowledge Graph** | The knowledge graph is enriched with inferred knowledge. The graph now contains both asserted and inferred triples. |
| **$R$ -- Reasoning & Rules** | **Primary contribution**. The reasoning layer is established. The ontology now has an active inference engine. |
| **$\Theta$ -- Trigger** | Inferred classification provide the conditions for triggers. A trigger on `HighRiskPatient` requires that `HighRiskPatient` be correctly classified by the reasoner. |
| **$\Phi$ -- Execution** | Execution decisions can now be based on inferred knowledge, not just asserted knowledge. This enables more intelligent and adaptive execution. |
| **$\Gamma$ -- Governance** | The reasoner enforces governance rules automatically. Disjointness axioms, domain/range constraints, and cardinality restrictions are all checked during classification. |

### 23.11.3 From $K$ to $R$: The Transition to Intelligence

Before **Stage 6**, the knowledge graph is **static** -- it contains only what was explicitly asserted by the engineer.

After **Stage 6**, the knowledge graph is **dynamic** -- it contains inferred knowledge that the reasoner derived.

This is the transition from **"data"** to **"intelligence**.

The reasoner is th engine that drives this transition. It takes the asserted axioms and computes their logical consequences. The result is a knowledge graph that is **richer**, **more connected**, and **more useful** than the sum of its asserted parts.

**Key Insight:**

> "The $K$ layer is the vocabulary. The $R$ layer is the grammar. Together, they produce meaningful sentences -- inferred knowledge that was never explicitly written (asserted)."

### 23.11.4 Reasoning as the Bridge to Execution

**Reasoning ($R$)** is the bridge between **knowledge ($K$)** and **triggering / execution ($\Theta$, $\Phi$)**.

Without reasoning, execution can only act on asserted knowledge. A trigger can only fire on conditions that were explicitly written. An execution rule can only reference facts that were explicitly asserted.

With reasoning, execution can act on inferred knowledge -- knowledge that the reasoner discovered itself. A trigger can fire on classifications that were derived by the reasoner. An execution rule can reference inferred facts.

This is what makes an ontology "**executable**": it can drive action based on logic, not just data.

**Key Insight:**

> "Reasoning is the engine that transforms a knowledge graph into a decision-making system. Without it, the ontology is a static library. With it, ontology is a proactive advisor."

## 23.12 Engineering Guidelines for Semantic Reasoning

The following guidelines distill the perspectives presented in this chapter into practical engineering practices.

| Guideline | Description |
| --- | --- |
| **Use Primitive Classes During Early Modeling** | Use primitive class (`SubClassOf`) for concepts that are still evolving. Convert to defined classes (`EquivalentTo`) once the concept has stabilized. |
| **Convert to Defined Classes When the Definition Is Stable** | Convert a primitive class to a defined class only after the definition has stabilized and the engineer is confident that it captures the intended meaning. |
| **Validate Before Reasoning** | Always validate first. Reasoning on an inconsistent ontology is dangerous. Ensure that **Stage 5 (Validation)** has been completed before relying on reasoning results. |
| **Reason Frequently, Not Just at the End** | Synchronize the reasoner after every significant change. Treat it as a design assistant, not a final exam. Frequent reasoning catches errors early. |
| **Design for Auto-Classification** | Design with the reasoner in mind. Ask: *"What do I want the reasoner to infer automatically?* Then write the definitions that make those inferences possible. |
| **Use the Explanation Feature** | When the reasoner produces an unexpected inference, use the explanation feature to understand why. This is the fastest way to diagnose modeling errors. |
| **Document Inferred Knowledge** | Document not just what the ontology asserts, but what it should infer. This makes the ontology's intended behavior explicit. |
| **Test with Multiple Reasoners** | Test your ontology with more than one reasoner. If reasoners disagree, investigate why. Document which reasoner you used and why. |
| **Choose the Right Reasoner for the Task** | Use Pellet or HermiT for DL ontologies, ELK for EL ontologies, Jena for RDF/SPARQL workflows, and Konclude for high-performance production. |
| **Apply Reasoning to Industrial Problems** | Use reasoning for error detection, safety compliance, risk management, and decision support. Reasoning is not just a technical feature -- it is a strategic capability. |

## 23.13 Key Concepts

| Concept | Description |
| --- | --- |
| Semantic Reasoning | The process of deriving new, logically entailed knowledge from an existing ontology through automated inference |
| Asserted Knowledge | Statements explicitly written in the ontology by the ontology engineer |
| Inferred Knowledge | Statements derived by the reasoner from the asserted axioms |
| Classification | The process of computing all inferred subclass relationships |
| Primitive Class | A class defined only through necessary conditions (`SubClassOf`); does not enable auto-classification |
| Defined Class | A class defined through necessary and sufficient conditions (`EquivalentTo`); enables auto-classification |
| Subsumption | The mathematical relation $\text{C} \sqsubseteq \text{D}$, meaning every instance of $\text{C}$ is also an instance of $\text{D}$ |
| Entailment | The relation $\mathcal{O} \models \phi$, meaning $\phi$ is true in every model of $\mathcal{O}$ |
| Model-Theoretic Semantics | The mathematical framework defining the meaning of OWL axioms |
| Interpretation | A structure $I = (\Delta^I, \cdot^I)$ consisting of a domain and an interpretation function |
| Model | An interpretation that satisfies all axioms of an ontology |
| Asserted Hierarchy | The class hierarchy as explicitly written by the engineer |
| Inferred Hierarchy | The class hierarchy as computed by the reasoner |
| Tableau Algorithm | The decision procedure used by OWL reasoners to check consistency and compute classification |
| Decidability | The property that reasoning is guaranteed to terminate with a definitive answer |
| NExpTime | The worst-case complexity class for OWL DL reasoning |
| PTIME | The complexity class for OWL EL reasoning |
| Pellet | A complete OWL 2 DL reasoner with incremental reasoning and explanation support |
| HermiT | A complete OWL 2 DL reasoner with hypertableau algorithm and strong explanation support |
| ELK | A highly optimized OWL 2 EL reasoner |
| JFact | A Java port of FaCT++, complete OWL 2 DL reasoner |
| Konclude | A high-performance C++ OWL 2 DL reasoner |
| Openllet | An open-source fork of Pellet |
| Jena | An Apache RDF framework with rule-based reasoning |
| DotNetRDF | A .NET RDF framework with basic reasoning |
| $R$ -- Reasoning & Rules Layer | The EKA layer established by SKDL Stage 6 |
| Automated Reasoning | The application of formal logic to derive conclusion from known conditions |
| Industrial Reasoning | The application of logical reasoning to industrial problem-solving, safety, and compliance |

## 23.14 Chapter Summary

This chapter introduced **Stage 6 -- Semantic Reasoning** of the Semantic Knowledge Development Lifecycle (SKDL), making the transition from verified knowledge to intelligent knowledge.

In **Chapter (22), we established **Semantic Validation** as the discipline of verifying that an ontology is logically consistent and fit for purpose. Validation gave us **trust**. But trust alone does not product **intelligence**.

This chapter addresses that gap by introducing **Semantic Reasoning** as the discipline of deriving new knowledge from a validated ontology.

Using **Exercise 20** and **Exercise 21** from Michael DeBellis' *Protégé 5 New OWL Pizza Tutorial* as practical case studies, we explored the critical distinction between primitive classes and defined classes.

- **Exercise 20** created `CheesyPizza` as a primitive class using `SubClassOf`. The reasoner did not auto-classify any pizzas as `CheesyPizza`, because primitive classes express only necessary conditions.
- **Exercise 21** converted `CheesyPizza` to a defined class using `EquivalentTo`. The reasoner immediately auto-classified `MargheritaPizza` and other cheese-topped pizzas as subclasses of `CheesyPizza`, because defined classes express necessary and sufficient conditions.

We also explored the landscape of variable RDF reasoners beyond Protégé:

- **Pellet** and **HermiT** are complete OWL 2 DL reasoners with strong explanation support
- **ELK** is a highly optimized reasoner for OWL 2 EL ontologies
- **JFact** is a Java port of FaCT++
- **Konclude** is a high-performance C++ reasoner for large ontologies
- **Jena** and **DotNetRDF** provide RDF-centric reasoning in the Java and .NET ecosystems

We compared reasoning results across different reasoners and explained why differences occur -- and what they tell us about the ontology's expressivity and the reasoner's capabilities.

We then examined reasoning from multiple complementary perspectives:

- **Mathematical perspective** -- model-theoretic semantics, subsumption, entailment, the tableau algorithm, decidability, and computational complexity
- **Historical perspective** -- from Aristotle's syllogism through Leibniz's dream of mechanical reasoning to modern OWL reasoners
- **Engineering perspective** -- reasoning as automated knowledge discovery, with the reasoner as a design assistant
- **Architectural perspective** -- reasoning as the bridge between knowledge ($K$) and triggers/execution ($\Theta$, $\Phi$) in the EKA framework
- **Industrial perspective** -- reasoning for problem-solving, decision-making, error detection, risk management, safety, and compliance

Within the EKA framework, **Stage 6** establishes the $R$ -- **Reasoning & Rules** layer. The knowledge graph is no longer static -- it now contains both asserted and inferred knowledge. The reasoner is the engine that drives this transformation.

The progression established across the first six stages of SKDL can therefore be summarized as:
...
| Stage 2 -- Semantic Description | Enrich concepts with formal semantic meaning |
| Stage 3 -- Knowledge Reuse | Transform validated semantic knowledge into reusable assets |
| Stage 4 -- Semantic Governance | Maintain semantic integrity through policies and standards |
| Stage 5 -- Semantic Validation | Verify correctness before deployment |
| Stage 6 -- Semantic Reasoning | Derive new knowledge through automated inference |

Together, these six stages establish both the semantic foundation and the intelligence required for trustworthy knowledge systems.

> **The reasoner is the engine of this intelligence.**

## 23.15 Looking Ahead -- Tower Executable Knowledge

By the end of this chapter, the `Pizza` ontology is not only validated but also intelligent. The reasoner has auto-classified `MargheritaPizza` and other pizzas under `CheesyPizza`. The inferred hierarchy now contains knowledge that was never explicitly asserted.

But intelligence is not the end. It is the beginning of **action**.

Now, with a validated and intelligent ontology, we can ask the question that has been waiting so long since the beginning of our journey:

> ***"How do we act on this knowledge?"***

This is the purpose of **Stage 7 -- Executable Knowledge** of the Semantic Knowledge Development Lifecycle (SKDL).

**Stage 7** addresses the operationalization of semantic knowledge -- connecting the ontology to **triggers ($\Theta$)**, **execution ($\Phi$)**, and the systems that act on knowledge. While reasoning asked *"What new knowledge can we derive?"*, execution asks *"What actions should we take based on that knowledge?"*

**Exercise 22** through **Exercise 36** introduce the building blocks of executable knowledge:
...
The next chapter therefore explores how professional ontology engineers transform semantic knowledge into executable intelligence -- the final stage of the Semantic Knowledge Development Lifecycle.

This progression reflects another enduring engineering principle:

> **Reasoning is the bridge between trust and intelligence. Execution is the bridge between intelligence and action.**
