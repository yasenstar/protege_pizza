...
Since we have moved `AmericanaHotPizza` and `SohoPizza` to further lower subclass level, e.g. `SohoPizza` now inherits the parent's disjointness and shows as below:
...
The reasoner will now flag an inconsistency if any individual is asserted to belong to two different subclasses of `NamedPizza`'s `MargheritaPizza` and `AmericanaPizza`.

Below sample, one pizza instance called `SamplePizza_BreakDisjoint` is created and pointed both `AmericanaPizza` and `MargheritaPizza` as Types, when synchronizing reasoner, the inconsistency error is pop up:

![ch21_07_ex18-07](img/ch21_07_ex18-07.png)

> Error message: An error occurred during reasoning
>
...
The full message can be read from the log as below:

![ch21_08_ex18-08-inconsistency-log](img/ch21_08_ex18-08-inconsistency-log.png)

When inconsistent occurs, your ontology class tree and many other places will be shown as RED, no need worry, this is the normal situation when you're working in ontology engineering.

Ontology is not crash, it's just give you the inconsistency notification, it's then up to you - the ontology engineer - to examine and decide what actions to be taken. Possible you need to remove one of the class types so that the rules built previous are followed, also it's possible that from this warning you may find the rule itself is not represent the true semantic -- say certain individual may correctly belong to these two classes at the same time -- then it's time to back examining and optimizing the rule.

Learning from those inconsistencies, your ontology can keep growing properly.
