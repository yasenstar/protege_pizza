...
- [14.8 Univeral Restriction -- Understanding `only`](#148-univeral-restriction----understanding-only)
  ...
  - [14.8.5 Combining `some` and `only` -- The Classis `VegetarianPizza` Pattern](#1485-combining-some-and-only----the-classis-vegetarianpizza-pattern)
    - [14.8.5.1 Why `some` Alone Is Not Enough?](#14851-why-some-alone-is-not-enough)
    - [14.8.5.2 Why `only` Alone Is Not Enough](#14852-why-only-alone-is-not-enough)
    - [14.8.5.3 Why Logical Compsition Matters](#14853-why-logical-compsition-matters)
  - [14.8.6 Implementing `some` and `only` in Graph Database - A Neo4j Perspective](#1486-implementing-some-and-only-in-graph-database---a-neo4j-perspective)
    ...
    - [14.8.6.4 Cypher Query vs. Pellet Reasoner vs. Open World Assumption](#14864-cypher-query-vs-pellet-reasoner-vs-open-world-assumption)
      ...
      - [Summary: The Logical Bridge Between Sections 14.8.6.1~14.8.6.3 and True OWL Reasoning](#summary-the-logical-bridge-between-sections-1486114863-and-true-owl-reasoning)
...
#### 14.8.5.1 Why `some` Alone Is Not Enough?
...
#### 14.8.5.2 Why `only` Alone Is Not Enough
...
#### 14.8.5.3 Why Logical Compsition Matters
...
### 14.8.6 Implementing `some` and `only` in Graph Database - A Neo4j Perspective
...
#### 14.8.6.1 Implementing Existential Restriction (`some`) in Neo4j
...
Sections 14.8.6.1 through 14.8.6.3 established a practical bridge between OWL restrictions and Cypher queries in Noe4j:

- Section 14.8.6.1 demonstrated how to implement existential restriction (`some`) using pattern matching with `MATCH` and `WHERE` clauses.
- Section 14.8.6.2 showed the implementation of universal restriction (`only`) using `NOT EXISTS` to detect invalid relationships.
- Section 14.8.6.3 then combined both restrictions, illustrating how complex semantic requirements -- such as "a pizza must have at least one cheese topping and no non-pizza toppings" -- can be expressed as graph patterns.
...
Second, and more critically, the Cypher implementations in 14.8.6.1~14.8.6.3 treat `some` and `only` as *query paggerns* that return results, whereas an OWL reasoner treats them as *logical axioms* that derive new knowledge.
...
New working ontology file: `/ebook/rdf/pizza-ebook_14.8.6.4.rdf`.

Base on above asserted facts, you may find following initialized knowledge:

![ch14_24_pizza-14.8.6.4](img/ch14_24_pizza-14.8.6.4.png)
...
![ch14_25_neo4j-14.8.6.4](img/ch14_25_neo4j-14.8.6.4.png)
...
![ch14_26_neo4j-14.8.6.4-add-hierarchy](img/ch14_26_neo4j-14.8.6.4-add-hierarchy.png)
...
Recall from 14.8.6.1 that the Cypher implementation of `some` uses a single pattern match:
...
| Individual | Cyper (CWA) - per 14.8.6.1 | Pellet Reasoner (OWA) | OWA Formal Conclusion |
| --- | --- | --- | --- |
...
Recall from 14.8.6.2 that the Cypher implementation of `only` uses `NOT EXISTS` to detect violations:
...
| Individual | Cyper (CWA) - per 14.8.6.2 | Pellet Reasoner (OWA) | OWA Formal Conclusion |
| --- | --- | --- | --- |
...
- The Cypher query from 14.8.6.2 correctly identifies valid pizzas (A and B) -- has explained result of A -- and excludes invalid ones (C and D). However, like the `some` case, it conflates `PizzaB` (vacuously true) with `PizzaA` (genuinely true), and cannot represent the logical distinction between "no toppings yet" and "has only valid toppings."

##### Part 3: Combining `some` and `only` -- The Composite Pattern

Section 14.8.6.3 demonstrated how to combine both restrictions in a single Cypher query, idenifying pizzas that satisfy *both* conditions: at least one cheese topping AND no non-pizza toppings.
...
- In the composite query from 14.8.6.3, `PizzaD` is correctly excluded.
...
The following table integrates both restriction types, showing the classification outcomes for each individual under each condition, comparing Cypher (from 14.8.6.1~14.8.6.3) against OWL reasoning:
...
The Cypher implementations in 14.8.6.1~14.8.6.3 are *operational filters*. They answer the questions:
...
| Dimension | Cypher in Neo4j (14.8.6.1~14.8.6.3) | OWL Reasoner (e.g. Pellet) |
| --- | --- | --- |
...
| --- | --- | --- |
| **Real-time filtering on stable complete data** | Use Cypher patterns (14.8.6.1~14.8.6.3) directly on Neo4j | <li>High performance</li><li>Closed world assumption is valid when data is known to be complement</li> |
...
##### Summary: The Logical Bridge Between Sections 14.8.6.1~14.8.6.3 and True OWL Reasoning

Let's take a breathe before moving to next section.

> The Cypher patterns in 14.8.6.1~14.8.6.3 answers:
...
The implementations in 14.8.6.1 (`some` as `MATCH`), 14.8.6.2 (`only` as `NOT EXISTS`), and 14.8.6.3 (composite patters) are excellent pedagogical (and self-learning) tools.
...
As you move from writing Cypher queries (sections 14.8.6.1~14.8.6.3) to deploying OWL reasoners in executable knowledge architectures (EKA), this distinction will determine whether your semantic system merely filters data or truly understands it.
