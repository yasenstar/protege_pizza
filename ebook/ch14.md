...
Recall different Pizzas we have modeled with restrictions in previous sections, putting them in below table can show the actions triggered by states clearly:

| Scenario | Data State | Reasoning Conclusion | Execution Action |
| --- | --- | --- | --- |
| `PizzaA` | Has cheese topping | `CheesePizza` (True) | Normal processing |
| `PizzaB` | No topping information | Unknown | Trigger data quality workflow, request completion |
