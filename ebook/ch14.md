...
Within the `Pizza.owl` tutorial, existential restriction marks an important turning point.

Up until this moment, ontology primarily behaved as:

> a structured semantic model.

- Classes existed.
- Relationships existed.
- Semantic boundaries existed.

However, ontology still lacked:

> formal semantic requirements.

Michale therefore introduces:

> **Existential Restriction**

through one of OWL's most widely used constructs `someValuesFrom`.

This allows ontology engineers to begin defining:

> what must exist

for a concept to satisfy its meaning.

Instead of merely creating `CheesePizza` as a named category, ontology can now define it logically:

> a pizza that has at least one cheese topping.

This changes ontology fundamentally.

Meaning no longer depends entirely on:

> manual classification.

Instead:

> ontology becomes inferable.

Reasoners may now determine:

> class membership automatically, base on the logical rules.

Ontology therefore begins transitioning from:

> manual curated categories

toward:

> **semantic intelligence.**

In the next section, you will perform **Exercise 13** to configure existential restriction directly inside Protégé and observe how reasoning begins tranforming semantic meaning into:
