...
Every object property ultimatedly becomes an RDF statement.
...
This is precisely why ontology natually evolves into graph databases.
...
Graph databases such as Noe4j natually operationalize these relationships.

Continue above example:

`Pizza` $\rightarrow$ `hasTopping` $\rightarrow$ `Cheesetopping`

Using Cypher syntax to translater above sample, is like below
...
> Which business capabilties depend on a deprecated system?
...
5. Explore how domain and range define semantic bounaries
...
These kind of questions gradually shift you from simply using Protégé to thiking like an ontology engineer.
...
<h3>Mistake 1 -- Confusing Hierarchy with Relationships</h3>
...
<h3>Mistake 2 -- Weak Relationship Naming</h3>
...
<h3>Mistake 3 -- Ignoring Domain and Range</h3>
...
<h3>Mistake 4 -- Overengineering Too Early</h3>

Another common mistaks is creating excessive complexity prematurely.
...
Ontology is no long simply organizing concepts.
...
Within the EKA roadmap, this chapter (10) makes the moment where ontology begins transitioning natually toward:
...
If object properties help ontology express relationships in one direction, then inverse properties allow machines to understand relationships from the oppsite perspective.

For example:

If:

> Pizze hasTopping CheeseTopping
